...
def replaceDataRegex(df, regexReplace):
    for i in range(len(regexReplace)):
        print(i)
        df['n_operateur'].replace(regexReplace[i]['valueMatch'], regexReplace[i]['replacerValueBy'], regex=True, inplace=True) # exemple de regex

# def dropDataWithCondition()
...
df.drop(['n_amenageur', 'source', 'date_maj'], axis=1, inplace=True)  # drop column, inplace true for reasign the dataFrame for df
